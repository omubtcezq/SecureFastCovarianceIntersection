...
%DIF DEL main_submitted_1.tex   Sat May 16 12:21:20 2020
%DIF ADD main.tex               Sat May 16 09:25:58 2020
...
\cite{lagendijkEncryptedSignalProcessing2012,alexandruEncryptedCooperativeControl2019,aristovEncryptedMultisensorInformation2018}}\hspace{0pt}%DIFAUXCMD
...
\cite{alexandruEncryptedCooperativeControl2019,aristovEncryptedMultisensorInformation2018}}\hspace{0pt}%DIFAUXCMD
...
\DIFaddend Both encryption schemes in sections \ref{subsec:paillier} and \ref{subsec:lewi} are defined over positive integers, and the Paillier scheme \DIFdelbegin \DIFdel{with the upper bound $N$ to the size of an encryptable integer }\DIFdelend \DIFaddbegin \DIFadd{bounds the largest encryptable integer by $N-1$}\DIFaddend . Due to the prevalence of real numbers in estimation theory, \DIFdelbegin \DIFdel{typically stored as floating-point numbers in modern-day hardware, an }\DIFdelend integer encoding of real \DIFdelbegin \DIFdel{number values is required for their encryption. This requires handling both negative and fractional numbers . 
}\DIFdelend \DIFaddbegin \DIFadd{numbers is an active field of research that acommpanies encrypted processing \mbox{%DIFAUXCMD
...
\cite{ziadCryptoImgPrivacyPreserving2016} }\hspace{0pt}%DIFAUXCMD
leak exponent information of the encrypted real number. We have instead relied on the simpler encoding in \mbox{%DIFAUXCMD
...
\DIFdel{. Fractional numbers are represented as integers using the quantising Q number format \mbox{%DIFAUXCMD
\cite{oberstarFixedPointRepresentationFractional2007}}\hspace{0pt}%DIFAUXCMD
. 
The encoding of a real number $a$, with maximum integer bits$i$ and fractional bits $f$ is represented by an $i+f$ bit long integer $e$, such that the maximum encoding is }\DIFdelend \DIFaddbegin \DIFadd{We consider encoding real numbers representable as rational fixed-point numbers of $b$ bits, consisting of a single sign bit, $i$ integer bits, and $f$ fractional bits. Thus, each encodable rational number is defined by its $b=1+i+f$ bits. As in \mbox{%DIFAUXCMD
\cite{farokhiSecurePrivateControl2017}}\hspace{0pt}%DIFAUXCMD
, encoding is performed to allow for multiplication, which requires an operation modulus of $b+2f$ to avoid the requirement for comparisons. Conversion of any real number $a$ to an encoded rational fixed-point number is }\DIFaddend given by
\DIFdelbegin \DIFdel{$2^{(i+f)}-1$. Encoding is performed by
}\DIFdelend \begin{equation}
   e = \floor{2^f a}\DIFaddbegin \pmod{2^{b + 2f}}\DIFaddend \,. \label{eqn:qmn}
\end{equation}
\DIFdelbegin \DIFdel{While encoded Q numbers are consistent under addition, multiplication }\DIFdelend \DIFaddbegin \DIFadd{Multiplication of such encoded numbers }\DIFaddend requires a factor of $1/2^f$ to be removed. As \DIFdelbegin \DIFdel{homomorphic division is not supported by the Paillier encryption scheme, the number of multiplications performed on encrypted values must be bounded and handled when decoding. As will be shown in section \ref{sec:secfci}, our fusion method will always require that a single multiplication factor be removed and leads to the decoding of an integer $e$ to a real number $a$ being given by
}\begin{displaymath}
   \DIFdel{a = \frac{e}{2^{2f}}\,. \label{eqn:qmn_mult_decode}
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{shown in \mbox{%DIFAUXCMD
\cite{farokhiSecurePrivateControl2017}}\hspace{0pt}%DIFAUXCMD
, cases of encoded multiplication can be computed exactly when using Paillier encryption, however, FCI guarantees only one homomorphic multiplication which we handle when decoding for simplicity. Decoding is defined by
}\begin{equation}
   \DIFadd{a=
   \begin{cases}
...
\DIFaddbegin \DIFadd{and will be correct given only a single encoded multiplication has occurred.
}\DIFaddend 

\DIFdelbegin \DIFdel{While }\DIFdelend \DIFaddbegin \DIFadd{Since }\DIFaddend the largest encryptable integer is given by \DIFdelbegin \DIFdel{$N/2-1$}\DIFdelend \DIFaddbegin \DIFadd{$N-1$}\DIFaddend , the largest encodable real number must account for \DIFdelbegin \DIFdel{the additional multiplication factor in its encoding}\DIFdelend \DIFaddbegin \DIFadd{this}\DIFaddend . Thus, the integer \DIFdelbegin \DIFdel{and fractional }\DIFdelend bits $i$ and \DIFaddbegin \DIFadd{fractional bits }\DIFaddend $f$ must be chosen such that
\begin{equation}\DIFdelbegin \DIFdel{(2^{(i+f)}-1)^2 \leq \frac{N}{2}-1 }\DIFdelend \label{eqn:qmn_max}
...
\end{equation}
where we note that $\omega_2=1-\omega_1$ due to the CI requirement \eqref{eqn:ci_omega_sum_bound}. We also note that in \eqref{eqn:paillier_ci_cov} and \eqref{eqn:paillier_ci_estimate}, each resulting value will have exactly one \DIFdelbegin \DIFdel{Q }\DIFdelend encoding multiplication factor to remove, and can be decoded exactly by using \eqref{eqn:qmn_mult_decode}.
