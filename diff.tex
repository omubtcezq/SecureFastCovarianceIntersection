...
%DIF ADD main.tex               Mon May  4 19:17:22 2020
...
\cite{farokhiSecurePrivateControl2017}}\hspace{0pt}%DIFAUXCMD
, particular cases of encoded multiplication can be computed exactly when using }\DIFaddend the Paillier encryption scheme, \DIFdelbegin \DIFdel{the number of multiplications performed on encrypted values must be bounded and handled when decoding . As }\DIFdelend \DIFaddbegin \DIFadd{however, we simplify decoding by taking advantage of the FCI algorithm, which guarantees only a single homomorphic multiplication as }\DIFaddend will be shown in section \ref{sec:secfci}, \DIFdelbegin \DIFdel{our fusion method will always require that a single multiplication factor be removed and leads to the decoding of an integer $e$ }\DIFdelend \DIFaddbegin \DIFadd{and avoid computing an inversion and exponentiation modulo $N$ operation that would be otherwise neccessary.
}
...
\cite{farokhiSecurePrivateControl2017} }\hspace{0pt}%DIFAUXCMD
for multiplied encoded numbers not to overflow. We allow any amount of multiplicative overflow (from the single multiplication operation) by decoding positive integers from the range $[0,2^{2b})$ }\DIFaddend to a real number \DIFdelbegin \DIFdel{$a$ being given by
}\DIFdelend \DIFaddbegin \DIFadd{representable as a rational fixed-point number consisting of a single sign bit, $2i+1$ integer bits, and $2f$ fractional bits. Decoded real numbers are representable as rational fixed-point numbers of $2b$ bits rather than the encodable ones of $b$ bits. Decoding is defined as the conversion from the signed Q number format \mbox{%DIFAUXCMD
\cite{oberstarFixedPointRepresentationFractional2007} }\hspace{0pt}%DIFAUXCMD
and defined by
}\DIFaddend \begin{equation}
   \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{e}\DIFaddend =
   \DIFdelbegin \DIFdel{\frac{e}{2^{2f}}}\DIFdelend \DIFaddbegin \begin{cases}
      a = 2^{-2f}\left(e \pmod{2^{2b}}\right) & e<2^{b-1} \\
      a = 2^{-2f}\left( 2^{2b} - (e \pmod{2^{2b}}\right) & e\geq 2^{b-1}
...
\begin{equation}
   (2\DIFdelbegin \DIFdel{^{(i+f)}}\DIFdelend \DIFaddbegin \DIFadd{^{(1+i+f)}}\DIFaddend -1)^2 \leq \frac{N}{2}-1 \label{eqn:qmn_max}
\end{equation}
holds.
