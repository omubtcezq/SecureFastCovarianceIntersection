...
%DIF DEL main_submitted_1.tex   Mon May  4 19:32:25 2020
%DIF ADD main.tex               Thu May  7 15:09:13 2020
...
%DIFDELCMD <       \mathcal{P}_0 & -\mathcal{P}_1 & 0 & \cdots & 0 \\
%DIFDELCMD <       0 & \mathcal{P}_1 & -\mathcal{P}_2 & \cdots & 0 \\
...
From \eqref{eqn:fci_2sen_omega_sum_eq} we know that \DIFdelbegin \DIFdel{$\omega_0$ must satisfy
}\DIFdelend \DIFaddbegin \DIFadd{$\omega_1$ must satisfy
}\DIFaddend \begin{equation}
   \omega\DIFdelbegin \DIFdel{_0 }\DIFdelend \DIFaddbegin \DIFadd{_1 }\DIFaddend \tr(\mP\DIFdelbegin \DIFdel{_0}\DIFdelend \DIFaddbegin \DIFadd{_1}\DIFaddend ) = (1-\omega\DIFdelbegin \DIFdel{_0}\DIFdelend \DIFaddbegin \DIFadd{_1}\DIFaddend )\tr(\mP\DIFdelbegin \DIFdel{_1}\DIFdelend \DIFaddbegin \DIFadd{_2}\DIFaddend )\,. \label{eqn:secfci_2sen_intersect}
\end{equation}
If we reverse \eqref{eqn:sen1_ore_list}, we obtain the equivalent list
\begin{equation}
   [\mathcal{E}^R_{ORE}((1-\omega\DIFdelbegin \DIFdel{^{(0)}}\DIFdelend \DIFaddbegin \DIFadd{^{(1)}}\DIFaddend )\tr(\mP\DIFdelbegin \DIFdel{_1}\DIFdelend \DIFaddbegin \DIFadd{_2}\DIFaddend )),\dots,\mathcal{E}^R_{ORE}((1-\omega^{(p)})\tr(\mP\DIFdelbegin \DIFdel{_1}\DIFdelend \DIFaddbegin \DIFadd{_2}\DIFaddend ))] \label{eqn:sen2_ore_rev_list}
\end{equation}
...
\begin{figure}[tb]
   ...
   \end{center}
   \vspace{-10pt}
   \caption{Approximation of \DIFdelbeginFL \DIFdelFL{$\omega_0$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\omega_1$ }\DIFaddendFL with discretisation step-size $s=0.1$. Only comparisons between line points are used.}
   \vspace{-\baselineskip}
   \label{fig:2_sensor_sol}
\end{figure}
Consecutive $\omega^{(x)}$ and $\omega^{(x+1)}$ for which list comparisons differ can be used to estimate the true intersection, and \DIFdelbegin \DIFdel{$\omega_0$, by
}\DIFdelend \DIFaddbegin \DIFadd{$\omega_1$, by
}\DIFaddend \begin{equation}
   \omega\DIFdelbegin \DIFdel{_0 }\DIFdelend \DIFaddbegin \DIFadd{_1 }\DIFaddend \approx \frac{1}{2}(\omega^{(x)} + \omega^{(x+1)})\,. \label{eqn:secfci_2sen_omega}
\end{equation}
In the case a comparison returns equality, the exact value of $\omega^{(x)}$ can be taken to be \DIFdelbegin \DIFdel{$\omega_0$}\DIFdelend \DIFaddbegin \DIFadd{$\omega_1$}\DIFaddend .
...
\begin{equation}
   \DIFdelbegin %DIFDELCMD < \begin{bmatrix}
%DIFDELCMD <       a_0^{(0)} & a_1^{(0)} & a_2^{(0)} \\
%DIFDELCMD <       a_0^{(1)} & a_1^{(1)} & a_2^{(1)} \\
%DIFDELCMD <       1 & 1 & 1
%DIFDELCMD <    \end{bmatrix}%%%
\DIFdelend \DIFaddbegin \begin{bmatrix}
      a_1^{(1)} & a_2^{(1)} & a_3^{(1)} \\
...
\begin{equation}
   a\DIFdelbegin \DIFdel{_0}\DIFdelend \DIFaddbegin \DIFadd{_1}\DIFaddend \omega\DIFdelbegin \DIFdel{_0 }\DIFdelend \DIFaddbegin \DIFadd{_1 }\DIFaddend + \dots +a_n\omega_n + a_{n+1} = 0\,. \label{eqn:nsen_plane_eq}
\end{equation}
...
\begin{equation}
   \DIFdelbegin %DIFDELCMD < \begin{bmatrix}
%DIFDELCMD <       a_0^{(0)} & a_1^{(0)} & \cdots & a_n^{(0)} \\
%DIFDELCMD <       a_0^{(1)} & a_1^{(1)} & \cdots & a_n^{(1)} \\
%DIFDELCMD <       \vdots & \vdots & \ddots & \vdots \\
%DIFDELCMD <       a_0^{(n-1)} & a_1^{(n-1)} & \cdots & a_n^{(n-1)} \\
...
%DIFDELCMD <       [\mathcal{E}^L_{ORE}(\omega^{(0)}\tr(\mP_i)),\dots,\mathcal{E}^L_{ORE}(\omega^{(p)}\tr(\mP_i))],\ i\text{ even} \\
%DIFDELCMD <       [\mathcal{E}^R_{ORE}(\omega^{(0)}\tr(\mP_i)),\dots,\mathcal{E}^R_{ORE}(\omega^{(p)}\tr(\mP_i))],\ i\text{ odd}\,.
%DIFDELCMD <    \end{gathered}%%%
\DIFdelend \DIFaddbegin \begin{gathered} \label{eqn:sensor_lists}
