...
\end{equation}
Note that \eqref{eqn:ci_cov} and \eqref{eqn:ci_estimate} compute the fusion of the information matrix and information vectors defined in \cite{niehsenInformationFusionBased2002} and reduce the fusion to a simple weighted sum.
...
From the CI equations \eqref{eqn:ci_cov} and \eqref{eqn:ci_estimate} we can see that both fusion equations can be computed on PHE encryptions of sensor information matrices and information vectors, given valid values for each $\omega_i$. In the two-sensor case, homomorphic fusion is computed by
\begin{equation}
