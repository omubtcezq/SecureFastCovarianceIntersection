...
\end{equation}
Note that Sensor $0$ uses only $L$ ORE while sensor $1$ uses only $R$ ORE and that both lists are ordered. Lists \eqref{eqn:sen0_ore_list} and \eqref{eqn:sen1_ore_list} are sent alongside PHE encryptions of local information matrix and information vector estimates to the fusion centre which uses them to estimate the FCI values of $\omega_0$ and $\omega_1$.
...
The fusion centre can then use its values for $\omega_0$ and $\omega_1 = 1-\omega_0$ and the recieved PHE encryptions of local information matrices and information vectors to compute \eqref{eqn:paillier_ci_cov} and \eqref{eqn:paillier_ci_estimate}.
...
This can be visualised in the three-sensor case, which requires solving partial solutions
\begin{equation}
...
In the $n$-sensor case, we can similarly solve partial solutions by first using the method from section \ref{sec:secfci} to solve an equation with two paramaters $\omega_k$ and $\omega_{k+1}$ when letting all $\omega_i=0,\ i\neq k,k+1$, and can compute a remaining point for each $\omega_i=1,\ i\neq k,k+1$ with $\omega_j=0,\ l\neq i$. Perpendicular hyperplanes can then be similarly defined in the form 
\begin{equation}
...
As all ORE comparisons are done between sequential sensors, given by \eqref{eqn:fci_eq}, $L$ and $R$ ORE encryptions can be used to the same effect as for the two-sensor case. The ORE ordered list sent from each sensor is given by
\begin{equation}
   ...
   \end{gathered}
\end{equation}
When combining \eqref{eqn:sensor_lists} with PHE encryptions of local information matrices and information vectors, SecFCI can be computed entirely homomorphically by \eqref{eqn:n_sen_paillier_ci_cov} and \eqref{eqn:n_sen_paillier_ci_estimate}.
...
\end{figure}
Values of each $\omega_i$ over time have been plotted in Fig. \ref{fig:fci_secfci_omegas}. Due to discretisation, as can be expected, SecFCI values of $\omega_i$ stay constant but may jump with sufficient change in true FCI $\omega_i$ values, as can be seen near the start of the simulation.
\begin{figure}[tb]
   \vspace{-5pt}
   \begin{center}
