...
From the CI equations \eqref{eqn:ci_cov} and \eqref{eqn:ci_estimate} we can see that both fusion equations can be computed on PHE encryptions of sensor information matrices and vectors, given valid values for each $\omega_i$. In the two-sensor case, homomorphic fusion is described by
\begin{equation}
...
\begin{figure}[tb]
   ...
   \end{center}
   \caption{Approximation of $\omega_0$ with discretisation step-size $s=0.1$, where only comparisons between line points are used.}
...
\begin{itemize}
   \item (also single multiplication)
